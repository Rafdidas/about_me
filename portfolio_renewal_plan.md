...
4. 디자인 시스템·UX 엔지니어 계열 포지션에 맞춰 사용성과 유지보수성을 함께 고민하는 사람임을 증명한다.
...
### 디폴트 포트폴리오 방침 (2026-07-06 확정)

이 포트폴리오는 **특정 회사 전용이 아닌 기본(디폴트) 포트폴리오**다.

```text
- 사이트에 특정 회사 언급을 넣지 않는다. 도메인 예시(매장·단말기 등)도 중립 B2B 소재로 유지한다.
- 지원처별 강조점 조정이 필요하면 data/ 문구 교체로만 대응한다. 구조는 건드리지 않는다.
- 첫 지원처(마감 2026-07-20)는 일정의 기준일 뿐이며, 이후 모든 지원에 이 사이트를 그대로 사용한다.
- 외부 참고용 디자인 분석 자료(DESIGN.md 등)는 공개 저장소에 두지 않는다.
  적용된 토큰이 이미 중립화되어 있으므로 원본 분석 문서는 로컬에만 보관한다.
```
...
Case Studies 진입점 (제목 + 한 줄 요약 링크 2개 — 카드 섹션까지는 불필요)
Featured: 대표 프로젝트 2~3개 카드
CTA (Design System 보기 / GitHub 보기)
하단: 간단 소개 + 연락처 (About 페이지 대체)
```

Case Studies 진입점은 디자이너/PM 계열 심사자(Case 중심 독자)의 Home 내 입구다.
DS를 주력으로 두는 구성은 유지하되, Case로 가는 길이 GNB뿐이면 안 된다.
...
### 실무 연결 브릿지 (2026-07-06 추가 — 필수)

DS가 주력 콘텐츠가 되려면 "경력과 연결된 증거"로 읽혀야 한다. 연결 문구가 없으면
자작 데모는 "잘 만든 연습작"으로 역해석된다 (6년 경력자에게는 감점 방향).

```text
- DS Overview에 한 줄: 실무(CRM·건설 플랫폼 등 B2B 업무 화면)에서 반복하던 기준과
  화면 구조를 재구성한 것임을 명시
- Admin Table 예시에 한 줄: 실무에서 다루던 목록·조회 화면 구조의 재현임을 명시하고
  해당 Case Study로 링크
- 문구는 data/로 관리하고, 과장 없이 "재구성/재현" 수준의 표현을 유지한다
```
...
### 프로젝트 이미지 방침 (2026-07-06 추가)

카드당 대표 스크린샷 1장을 넣는다.

```text
- 근거: 심사자는 텍스트보다 이미지로 카드를 스캔한다. UI 직무 포트폴리오에서
  이미지 없는 프로젝트 카드는 "보여줄 화면이 없다"로 읽힐 수 있다.
- 기준: 핵심 UI가 보이는 데스크톱 화면 1장, webp, next/image, alt 필수
- 금지: 저품질 캡처, 여러 장 나열, 로고만 있는 placeholder
```
